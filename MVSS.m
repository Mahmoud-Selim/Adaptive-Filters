classdef MVSS
    ...
    methods
        function obj = MVSS(gamma, step_size, n_epochs)
            obj.gamma = gamma;
            obj.step_size = step_size;
            obj.n_epochs = n_epochs;
            obj.errors = zeros(n_epochs);
        end
        function [inst_error, obj] = step(obj, x, output_sys, old_error)
            ...
            obj.p = obj.beta * obj.p + (1 - obj.beta) * inst_error * old_error;
        ...
        function obj = update_stepsize(obj, error)
            obj.step_size = obj.alpha * obj.step_size + obj.gamma * (sum(obj.p.^2) / 5);
        end
        ...
        function obj = train(obj, dataset, results)
            ...
            for j=1:obj.n_epochs
                ...
                for i=1:size(obj.w_filter):size(dataset, 2)
                    ...
                    [inst_error, obj] = step(obj, dataset(i : i - 1 + size(obj.w_filter)), results(cnt), obj.error);
